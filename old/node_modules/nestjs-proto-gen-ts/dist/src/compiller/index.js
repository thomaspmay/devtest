...
class Compiller {
    ...
    output(file, pkg, tmpl) {
        ...
        this.walkTree(root);
        const results = handlebars_1.compile(tmpl)(root);
        const outputFile = this.options.output ? path_1.join(this.options.output, file.replace(/^.+?[/\\]/, '')) : file;
    ...
    generate(path, pkg) {
        const hbTemplate = path_1.resolve(__dirname, '../..', this.options.template);
        if (!fs_extra_1.existsSync(hbTemplate)) {
            throw new Error(`Template ${hbTemplate} is not found`);
        }
        const tmpl = fs_extra_1.readFileSync(hbTemplate, 'utf8');
